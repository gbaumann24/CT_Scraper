...
def extract_reviews(page_content):
    ...
    Extracts:
      ...
      - Comment, Advantages, and Disadvantages based on paragraph labels.
    """
    ...
    for review in review_container.find_all(recursive=False):
        ...
        if stars_wrapper:
            # The rating is in the direct following sibling with class "ms-1".
        ...
        # Extract text sections (Comment, Advantages, Disadvantages).
        review_paragraphs = review.find_all("p")
        comment = ""
        advantages = ""
        disadvantages = ""
        # Loop through paragraphs and search for labels.
...
def parse_industry_employee(raw_ie):
    ...
    Also removes unwanted characters such as the curly apostrophe (’).
    """
    # Remove the unwanted apostrophe from the whole string first.
    raw_ie = raw_ie.replace("’", "")
    if "," in raw_ie:
...
def sanitize_text(text):
    """
    Remove or replace any characters that might interfere with CSV formatting.
    Here we remove newlines, carriage returns, and tabs, and collapse extra spaces.
    """
    if not text:
        return ""
    text = text.replace("\n", " ").replace("\r", " ").replace("\t", " ")
...
def main():
    ...
    # Process all products from the start_index onward.
    products_to_process = reader[start_index:]
        ...
        for i, row in enumerate(tqdm(products_to_process, desc="Processing Products", unit="product"), start=start_index):
            ...
            try:
                ...
                for review in reviews:
                    ...
                    # Sanitize all string fields to remove problematic characters.
                    for key, value in review.items():
                        if isinstance(value, str):
                            review[key] = sanitize_text(value)
